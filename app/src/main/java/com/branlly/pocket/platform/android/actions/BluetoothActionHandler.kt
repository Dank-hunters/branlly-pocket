...
class AndroidBluetoothCapabilityResolver(
    ...
    override fun resolve(action: ShortcutAction.EnableBluetooth): BluetoothCapabilities {
        val capability = capabilityResolver.resolve(AndroidCapability.BLUETOOTH_CONTROL)
        val permission = capability.granted
        val adapter = appContext.getSystemService(BluetoothManager::class.java).adapter
        val state =
            if (permission &&
                adapter != null
            ) {
                runCatching { adapter.state }.getOrDefault(BluetoothAdapter.STATE_OFF)
            } else {
...
class EnableBluetoothWorkflow(
    ...
    ): ActionWorkflowStep<EnableBluetoothState> =
        when (state) {
            EnableBluetoothState.CHECKING_STATE -> {
                val capabilities = capabilityResolver.resolve(action)
                when {
                    !capabilities.adapterAvailable -> {
                        ActionWorkflowStep.Failed("Aucun adaptateur Bluetooth n’est disponible.")
                    }

                    capabilities.hasConnectPermission && capabilities.state == BluetoothAdapter.STATE_ON -> {
