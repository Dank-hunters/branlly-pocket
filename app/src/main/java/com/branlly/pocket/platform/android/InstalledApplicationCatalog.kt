...
class InstalledApplicationCatalog(private val context: Context) {
    fun load(): List<InstalledApplication> {
        val packageManager = context.packageManager
        val intent = Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER)
        return packageManager.queryLaunchers(intent)
            .asSequence()
...
class ApplicationLauncher(private val context: Context) {
    fun launch(packageName: String?): ApplicationLaunchResult {
