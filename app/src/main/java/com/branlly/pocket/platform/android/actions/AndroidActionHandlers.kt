...
class OpenApplicationHandler(
    ...
    override fun validate(
        ...
        return buildList {
            if (!PACKAGE_NAME.matches(packageName)) {
                add(ActionValidationError("invalid_package", "Le package Android est invalide."))
            } else if (!context.isPackageInstalled(packageName)) {
                add(ActionValidationError("missing_package", "L’application n’est pas installée."))
            ...
            if (uri != null &&
                ...
                add(ActionValidationError("invalid_media_uri", "Le lien média doit être une URL HTTPS valide."))
            }
        }
    }
    ...
    private fun isSafeHttps(raw: String): Boolean =
        runCatching {
...
class WaitForMediaPlaybackHandler(
            ...
            if (action.expectedTitle != null || action.expectedArtist != null || !action.acceptAnyPlayingMedia) {
                add(
                    ActionValidationError(
                        ...
                        "Le filtrage par titre ou artiste n’est pas encore pris en charge.",
                    ),
                )
            }
        }
    ...
    override suspend fun execute(
        ...
        return when (val result = waiter.waitForPlayback(packageName, action.timeoutMillis)) {
            ...
            MediaWaitResult.TimedOut -> {
                ActionResult.TimedOut("La lecture multimédia a expiré.")
            }

            MediaWaitResult.SessionAbsent -> {
                ActionResult.TimedOut("Aucune session multimédia cible n’a été détectée.")
            }

            MediaWaitResult.PermissionMissing -> {
                ActionResult.PermissionRequired(
                    "Autorisez l’accès aux notifications pour surveiller la lecture.",
                )
            }

            MediaWaitResult.ListenerUnavailable -> {
                ActionResult.Failed(
                    "Le service de surveillance multimédia est indisponible.",
                    recoverable = true,
                )
            }

            is MediaWaitResult.ServiceUnavailable -> {
                ActionResult.Failed(result.reason, recoverable = true)
            }

            is MediaWaitResult.Failed -> {
                ActionResult.Failed(result.reason, recoverable = true)
            }
        }
    }
    ...
    override fun validate(
        ...
        return buildList {
            ...
            if (adapters.none { it.supports(NavigationTarget(packageName)) }) {
                add(
                    ActionValidationError(
                        "unsupported_navigation_provider",
                        "Aucun adaptateur de navigation ne prend en charge cette cible.",
                    ),
                )
            }
        }
...
class SetVolumeHandler(
    ...
    override fun validate(
        ...
        buildList {
            ...
            if (percent == null) {
                add(ActionValidationError("runtime_value_unresolved", "Choisissez un volume."))
            } else if (percent !in 0..100) {
                add(ActionValidationError("invalid_percent", "Le volume doit être compris entre 0 et 100 %."))
            }
            if (action.restoreAfterExecution) {
                add(
                    ActionValidationError("unsupported_restore", "La restauration automatique du volume n’est pas prise en charge."),
                )
            }
        }
...
class SetBrightnessHandler(
    private val context: Context,
    private val capabilityResolver: AndroidCapabilityResolver = AndroidCapabilityResolver(context.applicationContext),
    ...
    override suspend fun execute(
        ...
        val capability = capabilityResolver.resolve(AndroidCapability.WRITE_SYSTEM_SETTINGS)
        if (!capability.granted) {
            return ActionResult.PermissionRequired(capability.explanation, Settings.ACTION_MANAGE_WRITE_SETTINGS)
        }
        val percent = (action.percent as InputValue.Fixed<Int>).value
        return runCatching {
    ...
    override suspend fun execute(
        ...
        runCatching {
            ...
            if (action.mode == SoundMode.DO_NOT_DISTURB) {
                val capability = capabilityResolver.resolve(AndroidCapability.NOTIFICATION_POLICY_CONTROL)
                if (!capability.granted) {
                    return ActionResult.PermissionRequired(capability.explanation, Settings.ACTION_NOTIFICATION_POLICY_ACCESS_SETTINGS)
                }
                this.context
...
class WaitHandler : ActionHandler<ShortcutAction.Wait> {
    ...
    override fun validate(
        ...
        if (action.cancellable) {
            ...
            listOf(ActionValidationError("unsupported_non_cancellable_wait", "Les attentes non annulables ne sont pas prises en charge."))
        }
