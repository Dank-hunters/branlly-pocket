...
class PersistentRoutineExecutionStateStore(
    context: Context,
) : RoutineExecutionStateStore {
    ...
    override fun waitForUser(continuation: RoutineContinuation): Boolean =
        synchronized(lock) {
            ...
            if (current.executionId != continuation.executionId || current.routineId != continuation.routineId ||
                ...
                (continuation.continuationKey != null && current.continuation?.continuationKey == continuation.continuationKey)
            ) {
        ...
        active.continuation?.let { continuation ->
            value.put(
                ...
                JSONObject()
                    ...
                    .put("continuationKey", continuation.continuationKey)
    ...
    private fun read(): ActiveExecution? =
        runCatching {
            val raw = preferences.getString(KEY_ACTIVE, null) ?: return null
            val value = JSONObject(raw)
            val continuation =
                value.optJSONObject("continuation")?.let { item ->
                    ...
                    RoutineContinuation(
                        ...
                        continuationKey =
                            if (item.has("continuationKey") &&
                                !item.isNull("continuationKey")
                            ) {
                                item.getString("continuationKey")
                            } else {
                                null
                            },
